...
const InternalApproval = () => {
  ...
  const [activeTab, setActiveTab] = useState('tpc');
  ...
  useEffect(() => {
    const loadTasks = async () => {
      try {
        ...
        const [tpcData, itData] = await Promise.all([
          fetchTpcTasks(),
          fetchMyItTasks()
        ]);
        ...
        setTpcTasks(tpcData);
        setMyItTasks(itData);
        console.log('Tasks loaded successfully:', tpcData.length, 'TPC tasks,', itData.length, 'IT tasks');
      } catch (error) {
  ...
  const handleToggleSelectAll = (checked: boolean) => {
    if (checked) {
      // Only select pending tasks from current tab
      const currentTasks = activeTab === 'tpc' ? tpcTasks : myItTasks;
  ...
  const handleTabChange = (value: string) => {
    setActiveTab(value);
    setSelectedTasks([]); // Clear selections when switching tabs
  };
    ...
    return (
      <MainLayout>
        <div className="mb-6">
          ...
          <p className="text-slate-500">Manage task approvals</p>
        </div>
      ...
      <div className="space-y-6">
        <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
          <TabsList className="grid w-full grid-cols-2">
            <TabsTrigger value="tpc">TPC Tasks</TabsTrigger>
            <TabsTrigger value="myit">myIT Tasks</TabsTrigger>
          </TabsList>
          
          <TabsContent value="tpc" className="space-y-6">
          ...
          <TabsContent value="myit" className="space-y-6">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-4">
                <div className="text-sm text-slate-500">
            ...
            <div className="text-center py-8 text-slate-500">
              myIT Tasks view coming soon...
              <div className="mt-2 text-sm">
                {myItTasks.length} tasks loaded from mock data
              </div>
            </div>
          </TabsContent>
        </Tabs>
